...
4. **5가지 합성 레이아웃**:
   ...
   - 📄 **A4 레포트**: 괘선 A4 용지에 제목·문제·핵심 개념·풀이·검산·결론 순서로 정리, 길면 여러 장 (PDF 저장 시 실제 A4)
   - 🗂️ **코넬노트**: 왼쪽 키워드 칸 · 오른쪽 필기 칸 · 아래 요약 칸
   - 모든 레이아웃은 PNG 또는 PDF로 저장할 수 있습니다
5. **6가지 풀이 방식**: 1타 강사 실전 압축 / 개념 정석 / 여러 가지 풀이법 비교 / 오답노트형 / 단계별 힌트형 / 탐구 레포트형
6. **Gemini API 연동**:
...
- **그림 위 직접 표시**: AI가 문제 그림의 좌표를 짚어 주면 원본 사진 위에 길이·각(θ)·형광펜 강조선·보조선·정답 체크(✓)·보기 동그라미/빗금을 손글씨로 그려 넣고, 그 아래에 풀이를 씁니다(`core/annotation_engine.py`). 강조선은 인쇄된 선에 자동으로 맞춥니다. 화면의 스위치로 켜고 끌 수 있습니다.
